...
export async function refreshInsightAlerts(db: Admin): Promise<{ raised: number; resolved: number }> {
  const since = new Date(Date.now() - 7 * 24 * 3600_000).toISOString();

  const [overdue, failedJobs, pending, gaps, staleSources] = await Promise.all([
    db
      .from("cases")
      .select("id", { count: "exact", head: true })
      .lt("deadline_at", new Date().toISOString())
      .in("status", ["received", "draft_prepared", "in_review", "changes_requested"]),
    db.from("knowledge_ingestion_jobs").select("id", { count: "exact", head: true }).eq("state", "failed"),
  ...
  const candidates: Candidate[] = [
    ...
    },
    {
      ...
      description: "Uploaded or fetched publications could not be read, so nothing from them can be quoted.",
      severity: "warning",
